...
public class MemberController {
    @Autowired
    MemberServiceInter service; // di (의존성 주입)
    //회원가입 버튼 누릴 시 실행
    ...
    public void sign(AptVO vo, Model model, HttpSession session) {
      ...
      System.out.println(vo);
      service.update(vo); //db에서 member_code값을 가지고 update문 실행해서 개인정보 내역 수정
...
//    	System.out.println("controller의 sunny > " + sunny);
    ...
    public void idck(MemberVO vo, Model model) {
        MemberVO vo2 = service.idck(vo); //  DB에 id값을 넣어서 아이디 중복이 있는지 확인하러감.
        String result = "no";
    ...
    public String everyinfo(MemberVO vo, Model model, HttpSession session) {
    	...
    	//System.out.println(vo);
    	List<MemberVO> list = service.all(vo); // 관리자 권한으로 회원들의 정보를 가져와서 list에 담기
